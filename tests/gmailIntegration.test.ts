...
// (the P0 fix): the guard throws before any auth/network, surfacing as a
...
test('sendEmail rejects a newline-injected recipient', async () => {
  await assert.rejects(
    () => new GmailIntegration().sendEmail('a@b.com\nBcc: evil@x.com', 'Subject', 'Body'),
    /line breaks are not allowed/i
  );
});

test('sendEmail rejects a CRLF-injected subject', async () => {
  await assert.rejects(
    () => new GmailIntegration().sendEmail('a@b.com', 'Subject\r\nX-Evil: 1', 'Body'),
    /line breaks are not allowed/i
  );
});
...
test('createDraft rejects a newline-injected recipient', async () => {
  await assert.rejects(
    () => new GmailIntegration().createDraft('a@b.com\nBcc: evil@x.com', 'Subject', 'Body'),
    /line breaks are not allowed/i
  );
});
